...
import data_prepare
main_folder = '/content/affect_net_4'
def prepare_data(n):
    (train_data, train_labels), (test_data, test_labels) = data_prepare.load_data(main_folder)
    ...
    #gotta have the nums of classes specified here too
    ...
    for i, label in enumerate(labelled_y) :
        labelled_y_vec[int(i), int(labelled_y[i])] = 1.0

    unlabelled_y_vec = np.zeros((len(unlabelled_y), num_classes), dtype=np.float)
    for i, label in enumerate(unlabelled_y) :
        unlabelled_y_vec[int(i), int(unlabelled_y[i])] = 1.0

    test_labels_vec = np.zeros((len(test_labels), num_classes), dtype=np.float)
    for i, label in enumerate(test_labels) :
        test_labels_vec[int(i), int(test_labels[i])] = 1.0

    print(labelled_x.shape,labelled_y_vec.shape, unlabelled_x.shape,unlabelled_y_vec.shape, test_labels_vec.shape )
    return num_classes, labelled_x, labelled_y_vec, unlabelled_x, unlabelled_y_vec, test_data, test_labels_vec
