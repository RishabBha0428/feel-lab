#all files were made in tf1 and converted into tf2
...
class TripleGAN_10(object) :
    def __init__(self, sess, epoch, batch_size, test_batch_size, unlabel_batch_size, z_dim, n, gan_lr, cla_lr, checkpoint_dir, result_dir, log_dir):
        self.sess = sess
        ...
        self.model_name = "TripleGANFINAL"     # name for checkpoint
        ...
        #y_dim = num of classes
        self.y_dim, self.data_X, self.data_y, self.unlabelled_X, self.unlabelled_y, self.test_X, self.test_y = emotion_data.prepare_data(n) # trainX, trainY, testX, testY
    ...
    def discriminator(self, x, y_, scope='discriminator', is_training=True, reuse=False):
        with tf.compat.v1.variable_scope(scope, reuse=reuse) :
            ...
            net = tf.keras.layers.Dropout(.2)(x)
            y = tf.reshape(y_, [-1, 1, 1, self.y_dim])
            #x = conv_concat(x,y)

            net = tfa.layers.WeightNormalization(tf.keras.layers.Conv2D(32,[3,3], activation = 'relu'))(net)
            net = tfa.layers.WeightNormalization(tf.keras.layers.Conv2D(32,[3,3], activation = 'relu',strides = 2))(net)
            net =  tf.keras.layers.Dropout(.2)(net)

            net = tfa.layers.WeightNormalization(tf.keras.layers.Conv2D(64,[3,3], activation = 'relu'))(net)
            net = tfa.layers.WeightNormalization(tf.keras.layers.Conv2D(64,[3,3], activation = 'relu'))(net)
            net =  tf.keras.layers.Dropout(.2)(net)

            net = tfa.layers.WeightNormalization(tf.keras.layers.Conv2D(128,[3,3], activation = 'relu'))(net)
            net = tfa.layers.WeightNormalization(tf.keras.layers.Conv2D(128,[3,3], activation = 'relu'))(net)

            net = tf.keras.layers.GlobalAveragePooling2D()(net)
            net = tf.keras.layers.Flatten()(net)
            #net = concat([x,y_]) # mlp_concat

            x_logit = tf.keras.layers.Dense(1)(net)
            out = sigmoid(x_logit)

            return out, x_logit, net

    def generator(self, z, y, scope='generator', is_training=True, reuse=False):
        with tf.compat.v1.variable_scope(scope, reuse=reuse) :

            net = concat([z, y]) # mlp_concat

            net = tf.keras.layers.Dense(512 * 4 * 4, activation ='relu')(net)
            ...
            net = tf.keras.layers.Conv2DTranspose(256, kernel_size=[5,5], strides=2, activation = 'relu')(net)
            net = tf.keras.layers.BatchNormalization()(net)

            net = tf.keras.layers.Conv2DTranspose(128, kernel_size=[5,5], strides=2, activation = 'relu')(net)
            net = tf.keras.layers.BatchNormalization()(net)

            net = tf.keras.layers.Conv2DTranspose(3, kernel_size=[5,5], strides=2)(net)
            net = tf.keras.activations.tanh()(net)

            return net
    def classifier(self, x, scope='classifier', is_training=True, reuse=False):
        with tf.compat.v1.variable_scope(scope, reuse=reuse) :
            net = tf.keras.layers.GaussianNoise()(net)# default = 0.15
            net = tfa.layers.WeightNormalization(tf.keras.layers.Conv2D(128,[3,3], activation = tf.keras.layers.LeakyReLU(alpha=0.01)))(net)
            net = tfa.layers.WeightNormalization(tf.keras.layers.Conv2D(128,[3,3], activation = tf.keras.layers.LeakyReLU(alpha=0.01)))(net)
            net = tfa.layers.WeightNormalization(tf.keras.layers.Conv2D(128,[3,3], activation = tf.keras.layers.LeakyReLU(alpha=0.01)))(net)

            net = tf.keras.layers.MaxPool2D(kernel_size=[2,2], stride=2)(net)
            net = tf.keras.layers.Dropout(.5)(net)

            net = tfa.layers.WeightNormalization(tf.keras.layers.Conv2D(256,[3,3], activation = tf.keras.layers.LeakyReLU(alpha=0.01)))(net)
            net = tfa.layers.WeightNormalization(tf.keras.layers.Conv2D(256,[3,3], activation = tf.keras.layers.LeakyReLU(alpha=0.01)))(net)
            net = tfa.layers.WeightNormalization(tf.keras.layers.Conv2D(256,[3,3], activation = tf.keras.layers.LeakyReLU(alpha=0.01)))(net)

            net = tf.keras.layers.MaxPool2D(kernel=[2,2], stride=2)(net)
            net = tf.keras.layers.Dropout(.5)(net)

            net = tfa.layers.WeightNormalization(tf.keras.layers.Conv2D(512,[3,3], activation = tf.keras.layers.LeakyReLU(alpha=0.01)))(net)
            net = tf.keras.layers.Dense(256)(net)
            net = tf.keras.layers.Dense(128)(net)

            net = tf.keras.layers.GlobalAveragePooling2D()(net)
            net = tf.keras.layers.Flatten()(net)
            net = tfa.layers.WeightNormalization(tf.keras.layers.Dense(self.y_dim, activation = tf.keras.layers.LeakyReLU(alpha=0.01)))(net)
            return net
    ...
    def build_model(self):
        ...
        self.alpha_p = tf.compat.v1.placeholder(tf.float32, name='alpha_p')
        self.gan_lr = tf.compat.v1.placeholder(tf.float32, name='gan_lr')
        self.cla_lr = tf.compat.v1.placeholder(tf.float32, name='cla_lr')
        self.unsup_weight = tf.compat.v1.placeholder(tf.float32, name='unsup_weight')
        self.c_beta1 = tf.compat.v1.placeholder(tf.float32, name='c_beta1')

        """ Graph Input """
        # images
        self.inputs = tf.compat.v1.placeholder(tf.float32, [bs] + image_dims, name='real_images')
        self.unlabelled_inputs = tf.compat.v1.placeholder(tf.float32, [unlabel_bs] + image_dims, name='unlabelled_images')
        self.test_inputs = tf.compat.v1.placeholder(tf.float32, [test_bs] + image_dims, name='test_images')

        # labels
        self.y = tf.compat.v1.placeholder(tf.float32, [bs, self.y_dim], name='y')
        self.unlabelled_inputs_y = tf.compat.v1.placeholder(tf.float32, [unlabel_bs, self.y_dim])
        self.test_label = tf.compat.v1.placeholder(tf.float32, [test_bs, self.y_dim], name='test_label')
        self.visual_y = tf.compat.v1.placeholder(tf.float32, [self.visual_num, self.y_dim], name='visual_y')

        # noises
        self.z = tf.compat.v1.placeholder(tf.float32, [bs, self.z_dim], name='z')
        self.visual_z = tf.compat.v1.placeholder(tf.float32, [self.visual_num, self.z_dim], name='visual_z')
        ...
        R_L = tf.reduce_mean(input_tensor=tf.nn.softmax_cross_entropy_with_logits(labels=tf.stop_gradient(self.y), logits=C_real_logits))
        ...
        R_P = tf.reduce_mean(input_tensor=tf.nn.softmax_cross_entropy_with_logits(labels=tf.stop_gradient(self.y), logits=C_fake_logits))

        #

        # get loss for discriminator
        d_loss_real = tf.reduce_mean(input_tensor=tf.nn.sigmoid_cross_entropy_with_logits(logits=D_real_logits, labels=tf.ones_like(D_real)))
        d_loss_fake = (1-alpha)*tf.reduce_mean(input_tensor=tf.nn.sigmoid_cross_entropy_with_logits(logits=D_fake_logits, labels=tf.zeros_like(D_fake)))
        d_loss_cla = alpha*tf.reduce_mean(input_tensor=tf.nn.sigmoid_cross_entropy_with_logits(logits=D_cla_logits, labels=tf.zeros_like(D_cla)))
        self.d_loss = d_loss_real + d_loss_fake + d_loss_cla

        # get loss for generator
        self.g_loss = (1-alpha)*tf.reduce_mean(input_tensor=tf.nn.sigmoid_cross_entropy_with_logits(logits=D_fake_logits, labels=tf.ones_like(D_fake)))

        # test loss for classify
        test_Y = self.classifier(self.test_inputs, is_training=False, reuse=True)
        correct_prediction = tf.equal(tf.argmax(input=test_Y, axis=1), tf.argmax(input=self.test_label, axis=1))
        self.accuracy = tf.reduce_mean(input_tensor=tf.cast(correct_prediction, tf.float32))

        # get loss for classify
        max_c = tf.cast(tf.argmax(input=Y_c, axis=1), tf.float32)
        c_loss_dis = tf.reduce_mean(input_tensor=max_c * tf.nn.softmax_cross_entropy_with_logits(logits=D_cla_logits, labels=tf.stop_gradient(tf.ones_like(D_cla))))
        ...
        with tf.control_dependencies(tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.UPDATE_OPS)):
            self.d_optim = tf.compat.v1.train.AdamOptimizer(self.gan_lr, beta1=self.GAN_beta1).minimize(self.d_loss, var_list=d_vars)
            self.g_optim = tf.compat.v1.train.AdamOptimizer(self.gan_lr, beta1=self.GAN_beta1).minimize(self.g_loss, var_list=g_vars)
            self.c_optim = tf.compat.v1.train.AdamOptimizer(self.cla_lr, beta1=self.beta1, beta2=self.beta2, epsilon=self.epsilon).minimize(self.c_loss, var_list=c_vars)
        ...
        """ Summary """
        d_loss_real_sum = tf.compat.v1.summary.scalar("d_loss_real", d_loss_real)
        d_loss_fake_sum = tf.compat.v1.summary.scalar("d_loss_fake", d_loss_fake)
        d_loss_cla_sum = tf.compat.v1.summary.scalar("d_loss_cla", d_loss_cla)

        d_loss_sum = tf.compat.v1.summary.scalar("d_loss", self.d_loss)
        g_loss_sum = tf.compat.v1.summary.scalar("g_loss", self.g_loss)
        c_loss_sum = tf.compat.v1.summary.scalar("c_loss", self.c_loss)
        ...
        self.c_sum = tf.compat.v1.summary.merge([d_loss_cla_sum, c_loss_sum])
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.epoch):
            ...
            summary_test = tf.compat.v1.Summary(value=[tf.compat.v1.Summary.Value(tag='test_accuracy', simple_value=test_acc)])
    ...
    def save(self, checkpoint_dir, step):
        ...
        self.saver.save(self.sess, os.path.join(checkpoint_dir, self.model_name + '.h5'), global_step=step)
